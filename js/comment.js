        ...
        button.addEventListener('click', async () => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    alert(result.message || 'Lỗi khi thực hiện hành động.');
                }
            } catch (error) {
            ...
            if (confirm('Bạn có chắc chắn muốn xóa bình luận này?')) {
                try {
                    ...
                    if (result.success) {
                        ...
                        alert(result.message || 'Lỗi khi xóa bình luận.');
                    }
                } catch (error) {
                    console.error('Error:', error);
                    alert('Đã xảy ra lỗi. Vui lòng thử lại.');
            ...
            actionsDiv.querySelector('.save-edit').addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        alert(result.message || 'Lỗi khi chỉnh sửa bình luận.');
                    }
                } catch (error) {
                    console.error('Error:', error);
                    alert('Đã xảy ra lỗi. Vui lòng thử lại.');
...
commentActions.querySelector('button.bg-blue-600').addEventListener('click', async () => {
    ...
    try {
        ...
        if (result.success) {
            commentTextarea.value = '';
            commentActions.classList.add('hidden');
            loadComments(); // Tải lại với thứ tự mặc định (desc)
        } else {
            alert(result.message || 'Lỗi khi lưu bình luận.');
        }
    } catch (error) {
        console.error('Error:', error);
        alert('Đã xảy ra lỗi. Vui lòng thử lại.');
